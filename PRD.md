...
Runtime   ( ) Connected      (•) Local session      ( ) Hosted      ( ) Custom
  ...
  Working dir     [ ~/dev/api                    ]   [ Choose… ]
  Harness         [ Claude Code ▾ ]  Claude Code · Codex · Open Coder · Custom command
  Launch command  [ claude --print --mcp-config <generated> …          ]   ↺ reset to preset
...
**The launch presets ship as server-delivered data, not compiled into the binary.** Harness CLIs change their flags; a Mac App Store release takes days of review. If the preset for `codex` lived in the app bundle, one upstream flag rename would break every local agent on macOS until Apple approved a fix. Presets are therefore versioned rows the runner fetches and caches, with the field editable per agent and a "reset to preset" affordance so a hand-edit is recoverable. This is a small decision that prevents a very bad week.

Each preset supplies a non-interactive invocation, an MCP config generated by Lepidy (pointing at this workspace, authenticated as the delegating owner, so the agent tools resolve ownership through the existing path and nothing new is invented), the brief passed as system-prompt text, and a **default permission posture that is the harness's safe one**. Loosening it is an explicit toggle whose label says what it means in plain words, not a flag buried in a command string.
...
3. **Directory allowlist.** The runner keeps its own list of directories a session may run in. A working directory set in the web UI that is not on the local list is refused, and the refusal is visible in both places.
...
- **The proxy tier (§8.3, Tier 0) is the primary path**, and this is the runtime that justifies building it. The agent calls Lepidy's proxy endpoint over MCP; Lepidy attaches the credential to an allowlisted host and returns the response. **The value never leaves our infrastructure**, which is the same guarantee injection gives, reached a different way.
- **Managed Agents has its own vault** — credentials stored by Anthropic and substituted at egress. Mirroring a Lepidy credential into it is *possible* and mostly **discouraged**: the plaintext leaves our custody, the credential now lives in two places, and rotation has to cover both. It is **refused outright for human-gated (Tier B) credentials**, since the whole point of that tier is that nobody but the human can unwrap it. Where a Tier-A credential genuinely can't go through the proxy, the mirror is available as a per-credential, explicitly-acknowledged choice that says all of the above on the toggle.

That difference — injection on your own machine versus proxy in the cloud — is the honest axis on which somebody picks `local` over `claude_cloud`, and the runtime picker says so in a sentence rather than making people work it out.
...
### 8.1 What changes when the vault leaves the laptop — state this before anything else

`agent-vault`'s threat model rests on three sentences: one process owns the keys, there is no network, and there is no account. **Lepidy discards all three.** The vault is now a multi-user, hosted service, and the honest consequence is:

> **Lepidy — the running Worker and whoever controls its key material — can decrypt any server-mediated credential in the workspace.** For those credentials, the product is asking you to trust an operator. That is a real change from a local vault, and it is the price of an agent being able to act while you are asleep and of a teammate being able to approve when you are not at your desk.

Two things follow, and both are requirements rather than reassurances:

1. **Two tiers of custody, and the UI never lets you confuse them.** Tier A credentials are server-mediated and the sentence above applies. Tier B credentials are additionally wrapped by a key derived in *your browser* from a passkey or passphrase; the server holds ciphertext it cannot open, and release requires a live human session. Tier B is the right home for a production key, and the product should say so at creation time.
2. **Everything else is a mitigation, not a substitute:** per-credential data keys, a workspace root key that never touches the database, values never logged anywhere, a hash-chained audit trail, TTLs and single-use grants that bound how long a leak stays useful, and a kill switch.
...
- Anyone with control of the Lepidy deployment and its root key, for Tier A credentials.
...
All primitives are WebCrypto-native so they run in a Worker with no WASM in the hot path.

```
  Workspace Root Key (WRK)          32B random, Cloudflare Secrets Store, per workspace.
    │                               Never written to D1. Rotatable without re-encrypting bodies.
    ├─ wraps ─▶ Workspace Data Key (WDK)      stored wrapped in D1
    │              │
    │              └─ wraps ─▶ per-credential DEK   stored wrapped beside the ciphertext
    │                              │
    │                              └─ AES-256-GCM(DEK, plaintext)
    │                                 AAD = workspace_id ‖ credential_id ‖ version
    │
  Tier B only — a second, independent wrap of the same DEK:
    passkey PRF  ──HKDF-SHA256──▶ KEK_user  ─┐
    passphrase   ──Argon2id(WASM,browser)──▶ ├──▶ wraps a copy of the DEK
                                             ┘    stored in D1; the server cannot unwrap it
```

- **A random DEK per credential, rotated on every value change.** Envelope encryption buys root-key rotation without touching bodies, and it bounds a single-key compromise to a single credential.
...
- **Argon2id runs in the browser, never on the Worker.** Workers' SubtleCrypto has PBKDF2 and not Argon2; more to the point, a server that never receives the passphrase cannot leak it.
- **The passkey PRF extension is the good path** for Tier B: it derives `KEK_user` from a device-bound authenticator with no passphrase to forget, and it is the hosted equivalent of `agent-vault`'s Touch ID. A passphrase remains available for authenticators without PRF, along with a printable one-time recovery code — and the recovery code's consequences must be stated at setup, because forgetting a Tier B passphrase without one is total loss of that credential.
...
The CLI authenticates as a registered device, asks Lepidy for the value, policy is applied server-side, and the CLI spawns the child process with the secret in its environment. The plaintext exists in the child's env for the life of that process and nowhere else. The agent sees only the command's output. This covers the overwhelming majority of real usage: `gh`, `aws`, `curl`, `psql`, `stripe`, `wrangler`, and every custom script.
...
**Tier 0 — `proxy`, and this is what hosting unlocks.** `agent-vault` deferred a credential-injecting HTTP proxy because doing it locally means MITM-ing TLS with a locally-trusted CA — a large amount of machinery and its own liability. **Hosted, that objection evaporates.** The agent calls a Lepidy endpoint *deliberately*:
...
The Worker attaches the credential to an **allowlisted host** and returns the response. No CA, no interception, no local trust store, and the plaintext never leaves Cloudflare. Per-credential the proxy carries its own allowlist (`api.stripe.com` and nothing else) and its own rate limit. This is the strongest form of "the agent never sees the secret" and it is the natural default for any credential that is only ever an HTTP header — which is most of them.
...
For Tier B credentials and any credential flagged high-risk, the Allow buttons require a **WebAuthn assertion with user verification** — a phishing-resistant, device-bound gesture that also unwraps the Tier B key. This is `agent-vault`'s Touch ID requirement, translated correctly.
...
**v1: a call button that isn't a lie.** It creates a link with whatever the workspace has configured — Meet, Zoom, Jitsi — posts it in the channel, and shows who has joined. It takes days rather than weeks and it removes the objection honestly instead of shipping a bad huddle. **Native huddles are v2**, and the reference app's WebRTC implementation is the starting point.
...
**The free tier is a complete single-player product, not a trial.** One human, unlimited agents, the entire vault — injection, policy, approvals, audit, the kill switch — and the entire agent collaboration surface. This is deliberate and it is the acquisition strategy: `agent-vault`'s functionality is worth installing on its own, it is worth installing *alone*, and the moment that person wants a second pair of eyes on an approval they need a second seat. Nothing is crippled to force that; the second human is simply the thing you cannot do.
...
**Resource allowances:** Solo includes 1 GB of attachments. Team includes 25 GB plus 5 GB per paid seat above five. Optional 100 GB packs cost $5/month, purchased explicitly. Warn before limits and block additional uploads rather than deleting files. All core security features, audit export, and offboarding remain included. Agent identities are unlimited; published request-rate and concurrency limits must be established through workload testing before launch. No automatic usage overages, token credits, or per-approval charges. Agent execution is paid directly by the customer to their provider. SQLite capacity and archive policy remain separate from attachment allowances.
...
**What happens when a plan lapses:** the workspace goes read-only for humans and **agent access switches off**, which is the kill switch's existing behaviour and needs no new machinery. Credentials are never deleted for non-payment. A vault that eats your keys when a card expires is not a vault.

---

## 12. MCP surface

Lepidy is a **remote MCP server** at `https://<workspace>.lepidy.app/mcp`, speaking Streamable HTTP, with an in-app OAuth 2.1 authorization server. Non-negotiable: PKCE-S256 for all clients, **RFC 8707 resource indicators** with audience-bound tokens verified at the resource server, RFC 9728 protected-resource metadata, RFC 8414 AS metadata, RFC 9207 issuer identification, and dynamic client registration. Access tokens ~1 hour; refresh tokens rotate. **The acting user is derived from the verified token and only from there** — no tool accepts a user id.
...
| **Tenant data** | **One SQLite-backed Durable Object per workspace** — messages, channels, agents, queue, vault ciphertext, audit, FTS5 index | Tenant isolation is *structural*: there is no shared table a missing `WHERE` could leak across. Decisive for a product holding other people's credentials. See [HLD](./HLD.md) §3. |
...
| Realtime | **The same workspace DO**, WebSocket Hibernation | The object that owns the data owns the sockets, so "durably written" and "everyone told" are one step, not a two-system consistency problem. |
...
| Root key custody | **Secrets Store** | The WRK is the one thing that must not be in D1. |
...
**The constraint worth designing for now, not discovering later:** a Durable Object's SQLite caps at **10 GB and cannot be raised.** For text that is on the order of ten million messages, which no 50-seat team reaches quickly — but it is finite. `databaseSize` is reported on a daily alarm, admins are warned at 80%, and the designed escape hatch is archiving cold messages to R2 while keeping their search index rows. Do not build it in v1; do not close the door on it.
...
**Postgres → SQLite port notes**, since the reference schema assumes Postgres: `pgSchema("chat")` disappears (SQLite has no schemas — use a table prefix or nothing); `uuid`/`gen_random_uuid()` become `text` with an application-generated UUIDv7 (which also gives us sortable ids, useful for keyset pagination); `jsonb` becomes `text` with JSON functions; `timestamptz` becomes integer epoch milliseconds; partial indexes survive as-is; `ON CONFLICT` survives as-is. Migrations are plain numbered `.sql` files applied in lexicographic order and made idempotent with `IF NOT EXISTS` guards, exactly as the reference does — that discipline is worth keeping.
...
- **A person's Google account is not ours to depend on.** They change employers, lose access to a Workspace domain, or move to a personal address. If Google is the identity, that person loses their agents, their credential ACLs and — for Tier B credentials (§8.2) — data nobody can recover. Making an outside party the single point of failure for a vault is not a decision we get to make on the user's behalf.
...
**Linking rule, and it is the one that gets products owned:** two login methods join to one account only when **both** sides present the same **verified** email. Google verifies its addresses, so linking there is safe today — but the rule is written against the provider we add next, not the one we have, because "link by whatever email the provider claimed" is the classic account-takeover primitive and it looks completely reasonable right up until it isn't.
...
Not exhaustive — the shape of what is new. **Two planes** ([HLD](./HLD.md) §5.1): the tables below live inside a workspace's own Durable Object, so they carry **no** `workspace_id` — there is no other tenant's data in the database to distinguish them from. Only the control-plane tables (accounts, sessions, workspaces, membership, devices, `subscriptions`) live in the global D1 and key on a workspace id.
...
CREATE TABLE credentials (
  ...
  dek_user_wrapped blob,                     -- tier B only; the server cannot unwrap this
...
CREATE TABLE agent_local_config (
  agent_id text PRIMARY KEY REFERENCES agents(id) ON DELETE CASCADE,
  runner_id text NOT NULL REFERENCES runners(id) ON DELETE CASCADE,
  working_dir text NOT NULL,
  harness text NOT NULL,             -- 'claude' | 'codex' | 'opencoder' | 'custom'
  preset_version integer,            -- which server-delivered preset this was built from
  launch_command text,               -- NULL = use the preset verbatim
  reuse_session integer NOT NULL DEFAULT 1,
  idle_timeout_s integer NOT NULL DEFAULT 900,
  max_concurrent integer NOT NULL DEFAULT 1,
  max_starts_per_hour integer NOT NULL DEFAULT 6,
  ...
  -- The runner keeps its OWN copy of what it will allow. This row is a request;
  -- the local policy is the authority. A wake message never carries a command.
...
CREATE TABLE harness_presets (       -- server-delivered, so a flag change is not an app release
  harness text NOT NULL, platform text NOT NULL, version integer NOT NULL,
  launch_command text NOT NULL, mcp_config_template text, notes text,
  PRIMARY KEY (harness, platform, version)
);
...
| **M6 — Hardening & ergonomics** | Tier B human-gated custody with passkey PRF, the egress proxy, `capture`, template mode, reveal-once, import, `scan`, tags, rate limits, rotation nags, recovery codes. | 2.5 weeks |
...
| **M9.5 — Local sessions** | The runner (in the desktop app and as `lepidy agentd`), the outbound DO socket, `agent_next(wait_ms)`, the session state machine and reuse rule, the drain check, the three harness presets as server-delivered data, the config screen, and the local security controls. Depends on M3 and M9. **This is the runtime most people will actually use.** | 2.5 weeks |
...
| 1a | Tenancy | **A workspace *is* a Durable Object.** Control plane in one D1; tenant content never shares a table | Isolation stops depending on every query being written correctly — the thing that would eventually leak a customer's credentials ([HLD](./HLD.md) §3.2). |
...
| 3 | Default disclosure | **Inject-only; reveal is per-credential opt-in; proxy where the shape allows** | Plus reveal-once as the human-initiated escape hatch. The single highest-value safety decision in the product. |
...
| 11 | Pricing | **Solo free (1 human) · Team $19 including 5 humans, +$4 per human above 5**, per month, per workspace | Agents are free; humans are the meter. The free tier is a complete product, which is the acquisition strategy. |
...
- **D2. ~~One workspace per deployment, or multi-tenant from day one?~~ Resolved: multi-tenant, one Durable Object per workspace** — see [HLD](./HLD.md) §3. Tenant content never shares a table, so isolation does not depend on every query being written correctly. What remains open is the workspace **URL shape** (subdomain vs path); recommended subdomain, because it makes the tenant explicit in every link somebody pastes. **Decide at M0.**
...
- **D7. Tier B without a device.** If a Tier B credential's only unwrapping key is on a passkey and that authenticator is lost, the recovery code is the only path back. Decide whether recovery codes are mandatory at Tier B creation (recommended: yes, and print it before the credential is saved) — the alternative is a support burden with no technical remedy.
- **D19. Do private statuses gate notifications?** Today they gate the feed, the counts, realtime and permalinks — but not who gets notified about an item that moves into one. That is the reference app's v1 answer and it is a real seam: an item moved into a private status can still have notified people who can no longer see it. Recommended: **align notifications with visibility before the first external workspace**, because "I got a notification about something I can't open" is a support ticket that reads like a security bug.
- **D20. Who may vote in a ranked room?** Every member, today. A queue used for prioritisation across a large workspace may want voting limited to a group. Not blocking; the shape is the same as the status allow-list.
...
- **D12. Does the free tier get local sessions?** They cost us almost nothing — the compute is the user's own machine, the tokens are their own subscription — so **yes**, and it is arguably the strongest thing the free tier can offer.
...
**The vault is hosted, so an operator can decrypt Tier A credentials** (§8.1). Mitigated by envelope encryption, a root key outside the database, Tier B for anything that matters most, and an audit trail — but not eliminated. Anyone who needs it eliminated wants `agent-vault`, and we should say so rather than pretend otherwise.
...
**Harness CLIs will change their flags and break local sessions.** Server-delivered presets and a weekly CI check against each harness's current release are the mitigation; a silent breakage that only manifests as "the agent stopped answering" is the failure to watch for, which is why a failed spawn posts in the room rather than logging quietly.
