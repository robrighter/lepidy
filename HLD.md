...
The cloud components authorize and relay; an enrolled, unlocked release device performs the credential-bearing fetch. Flow: the agent calls idempotent `proxy_request` over MCP → the workspace authority evaluates metadata policy and issues a one-minute request id → an encrypted request is durably queued and sent over that exact device's authenticated outbound runner socket → the device unwraps locally, checks the exact host and pinned public DNS answer, refuses redirects, performs the bounded fetch and redacts the response → an encrypted response returns through a signed device call → the workspace authority rechecks live authority and atomically records usage, grant consumption, result state and audit metadata. The first call returns `pending`; retrying the same key reads the result and never dispatches a second upstream request.

The request contains no server-selectable credential value or executable input. V1 follows no redirects and accepts only a narrow caller-header allowlist; the release device adds a fixed bearer authorization header. The normalized URL, headers and body are encrypted before durable storage or socket delivery. The cloud never receives the vault key, credential plaintext or an authorization header. No online release device means `vault_device_unavailable`, including for scheduled cloud agents; an unanswered delivered request becomes `uncertain` rather than being retried ambiguously.
