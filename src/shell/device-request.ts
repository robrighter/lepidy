/**
 * The signed-device transport, with nothing bound to a request context.
 *
 * Split out from `device-api` for one concrete reason: the runner's socket is
 * answered by the Worker entry rather than a Route Handler, and importing
 * `@opennextjs/cloudflare` from there pulls the adapter's own ambient
 * environment types into the Worker's program. Everything here works from
 * bindings the caller already holds.
 */
...
/**
 * The same environment, built from bindings the caller already holds.
 *
 * The runner's socket is served by the Worker entry rather than a route
 * handler — a Route Handler's response is generated and then the connection
 * closes, so a WebSocket cannot survive one — and out there `getCloudflareContext`
 * has no request to attach to. This is that path's way in, and it resolves to
 * exactly the same authorization service the signed routes use.
 */
export function deviceEnvironmentFrom(env: ShellEnvironment): DeviceEnvironment | null {
