...
export const channelHistory = cache(
  async (channelId: string): Promise<ChannelHistoryState> => {
    const environment = process.env.ENVIRONMENT ?? process.env.NODE_ENV;
    const token = (await cookies()).get(SESSION_COOKIE)?.value ?? null;

    if (!token) {
      return environment === "development"
        ? { status: "ready", page: developmentChannelHistory(channelId) }
        : { status: "not_found" };
    }
    ...
    return loadChannelHistory({ ...env, ENVIRONMENT: env.ENVIRONMENT ?? environment }, token, channelId);
  },
);
