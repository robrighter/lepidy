...
| V2 | `agent-vault@d794820`: CLI run, scrub, with-file, client and end-to-end tests | `src-tauri/crates/lepidy-cli/src/{run,scrub,withfile}.rs`; future runner crate | Adapted in V02: child spawn, split env/file deliveries, tty-based scrub default, chunk-boundary matcher, `Drop` cleanup and the distinct refusal exit code. The Unix-socket daemon is excluded and replaced by signed HTTPS with a local unwrap; Unix-only path and permission assumptions were replaced so the crate builds for Windows. |
