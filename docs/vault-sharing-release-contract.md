...
paths described below; V08, 2026-09-08 adds the scan targets and canary markers
in §2a, which are the only credential-derived values this contract permits the
cloud to hold besides ciphertext.
...
## 2a. Scan targets and canaries

Two credential-derived facts may be stored in cloud alongside the ciphertext,
both computed by the trusted client that sealed the value, and both optional.

A **scan target** is `SHA-256("lepidy-scan-v1" ‖ workspace ‖ credential ‖
version ‖ value)` plus the value's byte length. It exists so a client can walk
its own text and recognise a credential without the credential leaving the
vault, which is what makes `lepidy scan` and the `PreToolUse` hook possible at
all.

This is **new exposure that the ciphertext does not have, and it is accepted
deliberately rather than overlooked.** AES-GCM under a random per-version DEK
gives an attacker holding the database nothing to guess against; a digest gives
them a verifier for one exact value. The bounds on that trade are normative:

- No target is stored for a value shorter than eight bytes.
- A client may withhold one (`lepidy add --no-scan`), and a credential with no
  target simply never matches.
- The digest is bound to workspace, credential and version, so it cannot be
  tested against a value elsewhere or against a superseded version.
- A rotation replaces the target or clears it. A target describing a replaced
  value must never survive, because it would report a dead secret as live and
  miss the live one.
- Targets are served only over the signed device transport, and only to a member
  who already holds a verb on that credential.
- The claim is exact-whole-value detection. Encoded, escaped, split or partial
  forms are not detected, and no product copy may imply they are.

A **canary** is a fake credential whose value is `"lpdy-canary-" ‖ tag ‖ "-" ‖
random`, generated on a trusted client, never typed and never displayed. Its
marker — the prefix and tag — is stored in cleartext metadata. The marker is not
a secret: it exists so the workspace can spot the value in content it already
receives, with no key and no window scan, and a twelve-character random tag
behind a fixed prefix does not occur by accident. Marker detection is the trip;
the workspace never needs the value.
...
9. A scan target is absent below the minimum length and when withheld, is replaced or cleared by a rotation, and never matches a value from another credential, version or workspace.
10. A canary generated by a client is never printed, never appears in the create request outside the ciphertext, and is recognised by its marker alone wherever it later appears.
