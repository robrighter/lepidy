...
| `FORM-INT-001` | `tests/browser/productivity.spec.ts` | The paired case for the forms above: with scripting off, so the page can never hydrate, the agent and emoji submit buttons stay unavailable — which also removes the implicit submission that would navigate away — and the agent, emoji and composer fields report themselves as not editable rather than accepting characters the first render would wipe. |
...
| `MCP-INT-008–010` | `tests/browser/mcp-oauth.spec.ts` | The whole dance against the built Worker: discovery, the 401 challenge that names the workspace's own metadata, dynamic registration, a consent screen a person clicks, a code redeemed for a pair, the pair used at the workspace's MCP endpoint and reporting the connected handle, a redeemed code refused a second time, rotation with the superseded token dead, a replayed refresh token ending the connection, a disconnect from the connections page taking effect on the next request, and one workspace's token refused at another workspace while still working at its own. |
