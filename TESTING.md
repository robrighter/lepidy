...
| `FORM-INT-001` | `tests/browser/productivity.spec.ts` | The paired case for the forms above: with scripting off, so the page can never hydrate, the agent and emoji submit buttons stay unavailable and pressing Enter in a field neither navigates nor loses what was typed. |
