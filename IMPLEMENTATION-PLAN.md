...
Accepted decisions from the conversation and HLD take precedence over older milestone text. In particular: Next.js via OpenNext; a Team content DO or Solo metadata/relay DO with host SQLite; D1 control plane; customer-funded execution; authoritative authorization without KV session caching; local-only runner launch configuration; a client-held vault root/recovery code; Solo free and Team $19 including five humans plus $4 each above five.
...
| 5 | Complete custody and external runtimes | V05–V08, A05, R05 | Recovery, owner sharing, device-mediated proxy and cloud/custom lanes satisfy resolved contracts and leak tests |
...
Stage order is a delivery guide, not a reason to block independent work. Dependencies in the ledger govern readiness. Decision tasks can run ahead of their consuming stage. Vault policy prototypes may proceed while owner-sharing details are settled, but no implementation may introduce a server-decryptable wrap. Full desktop packaging does not block the early headless runner. Store-account setup and review preparation should start early, without making a public release.
...
| D02 | Queue and runner lifecycle | [Accepted contract](docs/runner-queue-contract.md): one designated runner and session per agent, 60-second fenced leases, pre-start retry versus ambiguous-outcome hold, local idle/concurrency limits, lost-wake recovery, disconnect stop and narrowly bound rotating session tokens |
| D03 | Waiting transport | [Accepted contract](docs/mcp-waiting-and-harness-contract.md): no parked wait; bounded calls answered immediately, a runner-held idle timer, a wake socket, and a queue-depth check that closes the lost-wake race; measured empty-queue, drain, recovery and stop latencies; per-harness permission posture certification belongs to R02/R04 |
| D04 | Vault authorization | [Accepted contract](docs/vault-authorization-contract.md): independent ACL verbs, union then mandatory intersection, one-owner delegation, authenticated origin, signed device/project claims, step-up matrix, digest-bound batch approvals and exact grant/revocation boundaries |
| D05 | Vault sharing and release | [Accepted contract](docs/vault-sharing-release-contract.md): client-held AVK and account wrapping key, per-custodian credential wraps, native-only root operations/reveal, encrypted phone-to-runner delivery, device-mediated proxy, rekey on removal and fail-closed recovery/loss |
| D06 | Cloud/custom contracts | [Accepted contract](docs/cloud-custom-runtime-contract.md): customer-workspace WIF without stored Anthropic API keys, pinned Managed Agents beta capability/lifecycle/budget rules, raw signed webhook verification plus reconciliation, and metadata-only custom wakes with HMAC replay and public-egress boundaries |
| D07 | Retention and operations | [Accepted contract](docs/retention-residency-recovery-contract.md): per-class expiry, resumable export/purge, 30-day recovery boundary, authority-safe restore, immutable default/EU profiles, audit anchors and authorized R2 archive search |
| D08 | Remaining product boundaries | **Accepted 2026-09-06**, all nine resolved — see the [D08 evidence record](IMPLEMENTATION-LEDGER.md#d08--product-and-commerce-decisions): prorated seat adds with removals at renewal; the accepted attachment allowance and packs confirmed unchanged against a separate unsellable 10 GB database ceiling; read-only lapse with export available and retention on D07's 30-day boundary; no purchase path in store builds; a companion that approves, denies and replies once in the thread; every member votes and an agent never does; notifications aligned with private-status visibility; files indexed by metadata not contents; deterministic Home and search. The call button was removed from MVP rather than resolved. |
| D08a | Free workspace storage boundary | Solo keeps workspace/channel metadata in its cloud relay object while one designated online computer owns content SQLite; specify routing, encrypted transit, offline behavior, host fencing and upgrade |
| D05a | Local execution and vault-root invariants | Local executable/script configuration has no remote mutation path and requires host OS user verification; vault unlock secret/recovery code are client-generated and never reach Lepidy storage |
...
- Next.js server components/actions/handlers call authorized backend methods. Team content is stored in its workspace DO; Solo content is stored only in the designated host SQLite database and traverses cloud relays as opaque authenticated ciphertext. D1 holds the control plane. Never put plaintext vault material in cloud stores, caches, logs or analytics.
...
- A runner uses executable/script/arguments/directory/environment/permission settings that exist only in protected local configuration. Remote surfaces expose no mutation schema for them. Signed app defaults change only through a local, OS-user-verified review. Avoid injecting application credentials into the entire harness environment; release them to the intended child command.
- A device-mediated proxy checks scheme/host/port, redirects, private destinations, credential header handling, body limits, upstream timeout and response leakage on the enrolled release device. Cloud services authorize and relay opaque payloads but never unwrap vault keys or credentials.
